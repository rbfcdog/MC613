...
entity tb_dram_demo is
end tb_dram_demo;

architecture sim of tb_dram_demo is
...
begin
  ...
  begin
    ...
    -- 1) Write 3 to address 0x01, read-back must show 3.
    do_write("000001", "0011");
    assert read_data = x"03"
      report "FALHA: escrita inicial nao leu 3 (leu " & integer'image(to_integer(unsigned(read_data))) & ")"
      severity failure;

    -- 2) Overwrite address 0x01 with 5: read-back MUST now show 5 (the bug).
    do_write("000001", "0101");
    assert read_data = x"05"
      report "FALHA: overwrite nao sobrescreveu (leu " & integer'image(to_integer(unsigned(read_data))) & ")"
      severity failure;

    -- 3) Write 7 to a different address 0x02.
    do_write("000010", "0111");
    assert read_data = x"07"
      report "FALHA: escrita em 0x02 nao leu 7" severity failure;

    -- 4) Change the read address back to 0x01: must show 5, not 7 (the bug).
    do_read("000001");
    assert read_data = x"05"
      report "FALHA: leitura de 0x01 nao mudou para 5 (leu " & integer'image(to_integer(unsigned(read_data))) & ")"
      severity failure;

    -- 5) Change the read address to 0x02: must show 7.
    do_read("000010");
    assert read_data = x"07"
      report "FALHA: leitura de 0x02 nao mudou para 7 (leu " & integer'image(to_integer(unsigned(read_data))) & ")"
      severity failure;

    report "tb_dram_demo: overwrite e troca de endereco OK" severity note;
    wait;
  end process;
end sim;
